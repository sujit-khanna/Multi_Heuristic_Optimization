...
class BATLevy:
    ...
    def __init__(self, obj, class_obj,num_agents, num_gen, seed, obj_dim, fmin=0.01, fmax=0.25, beta=1.5, pulse=0.2,levy_beta=1.5,amplitude=1,scaling=0.01,gamma=0.9,floor=-5, ceiling = 5,**kwargs):
        self.obj = obj
        ...
        for n in range(self.num_gen):
            ...
            for i in range(len(self.population)):
                ...
                if random.random()>self.r[i]:
                    population_copy[i] = self.current_best + self.levy*self.scaling*self.mean_amp
                    population_copy[i] = self.scale_params(population_copy[i])
...
if __name__ == '__main__':
    ...
    model = BATLevy(obj.rosenbrock_func, obj,num_agents=100, num_gen=2000,seed=10,obj_dim=params.shape[0])
